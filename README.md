...
数値を用いてセルを選択します。[変数を利用したセル](#forによる繰り返し選択)に向いています。基準は`A1`です。
...
End IF
```
ElseIF部は必須ではありません。必要に応じて消したり加えたりしてください。<br>
    [プロシージャ作成](#vbaの始め方)のEnd Subは自動入力されますが、**End IFは自動入力されません。**
